...
import { Verifier, deploy, expect, waitForTransaction } from "@gearbox-protocol/devops";
import {
  ERC20__factory,
  WAD,
  RAY,
...
const seedCoins = async function (coins: string[], poolAddress: string, signer: SignerWithAddress) {
  for (let coin of coins) {
    if (coin == ETH_ADDRESS) {
      ...
      await waitForTransaction(pool.donate_eth({ value: WAD.mul(300) }))
    } else {
      let token = ERC20Kovan__factory.connect(coin, signer);
      await waitForTransaction(token.mint(poolAddress, RAY))
    }
  }
}
...
describe("Curve mocks tests", async function () {
  ...
  const deployToken = async function (name: string, decimals: number) {
    return await deploy<ERC20Kovan>(
      "ERC20Kovan",
      log,
      name,
      name,
  ...
  it("Curve 3CRV mock test", async function () {
    ...
    await _3pool.add_liquidity([WAD.mul(100), USDC_UNIT.mul(100), USDC_UNIT.mul(100)], 0)
    ...
    expect(usdc_balance_after.sub(usdc_balance_before)).to.be.gt(USDC_UNIT.div(110))

    expect(await _3pool.balances(0)).to.be.eq(WAD.mul(200).add(WAD.div(100)))
    expect(await _3pool.balances(1)).to.be.lt(USDC_UNIT.mul(200).sub(USDC_UNIT.div(110)))
    ...
    await _3pool.remove_liquidity(WAD.div(100), [WAD.div(330), USDC_UNIT.div(330), USDC_UNIT.div(330)])
    ...
    expect(usdc_balance_after.sub(usdc_balance_before)).to.be.gt(USDC_UNIT.div(330))
    expect(usdt_balance_after.sub(usdt_balance_before)).to.be.gt(USDC_UNIT.div(330))
    ...
    await _3pool.remove_liquidity_imbalance([WAD, USDC_UNIT, USDC_UNIT.mul(2)], WAD.mul(9).div(2))
    ...
    expect(usdt_balance_after.sub(usdt_balance_before)).to.be.eq(USDC_UNIT.mul(2))
  ...
  it("Curve GUSD mock test", async function () {
    ...
    await _3pool.add_liquidity([WAD.mul(100000), USDC_UNIT.mul(100000), USDC_UNIT.mul(100000)], 0)
    ...
    console.log("Adding liquidity")
    await gusd3crv.add_liquidity([GUSD_UNIT.mul(100), WAD.mul(100)], WAD.mul(199))

    expect(await gusd3crv_token.balanceOf(deployer.address)).to.be.gt(WAD.mul(199))
    ...
    expect(_3crv_balance_after.sub(_3crv_balance_before)).to.be.gt(WAD.mul(9).div(10))
    ...
    expect(gusd_balance_before.sub(gusd_balance_after)).to.be.eq(GUSD_UNIT)
    expect(dai_balance_after.sub(dai_balance_before)).to.be.gt(WAD.mul(9).div(10))

    console.log("Removing liquidity")

    let gusd3crv_balance_before = await gusd3crv_token.balanceOf(deployer.address)
    gusd_balance_before = await gusd.balanceOf(deployer.address)
    _3crv_balance_before = await _3pool_token.balanceOf(deployer.address)

    await gusd3crv.remove_liquidity(WAD.div(100), [GUSD_UNIT.div(110), WAD.div(110)])

    let gusd3crv_balance_after = await gusd3crv_token.balanceOf(deployer.address)
    gusd_balance_after = await gusd.balanceOf(deployer.address)
    _3crv_balance_after = await _3pool_token.balanceOf(deployer.address)

    expect(gusd_balance_after.sub(gusd_balance_before)).to.be.gt(GUSD_UNIT.div(110))
    expect(_3crv_balance_after.sub(_3crv_balance_before)).to.be.gt(WAD.div(110))
    expect(gusd3crv_balance_before.sub(gusd3crv_balance_after)).to.be.eq(WAD.div(100))
    ...
    await gusd3crv.remove_liquidity_imbalance([GUSD_UNIT, WAD.mul(2)], WAD.mul(7).div(4))
    ...
    expect(gusd3crv_balance_before.sub(gusd3crv_balance_after)).to.be.lt(WAD.mul(7).div(4))
    ...
    expect(gusd_balance_after.sub(gusd_balance_before)).to.be.gt(GUSD_UNIT.mul(18).div(10))
  ...
  it("Curve metapool mock test", async function () {
    ...
    await frax3crv["add_liquidity(uint256[2],uint256)"]([WAD.mul(100), WAD.mul(100)], WAD.mul(199))
    ...
    await frax3crv["exchange(int128,int128,uint256,uint256)"](0, 1, WAD, WAD.mul(9).div(10))
    ...
    await frax3crv["exchange_underlying(int128,int128,uint256,uint256)"](0, 1, WAD, WAD.mul(9).div(10))
    ...
    await frax3crv["remove_liquidity(uint256,uint256[2])"](WAD.div(100), [WAD.div(110), WAD.div(110)])
    ...
    expect(_3crv_balance_after.sub(_3crv_balance_before)).to.be.gt(WAD.div(110))
    expect(frax3crv_balance_before.sub(frax3crv_balance_after)).to.be.eq(WAD.div(100))
    ...
    await frax3crv["remove_liquidity_imbalance(uint256[2],uint256)"]([WAD, WAD.mul(2)], WAD.mul(7).div(4))
    ...
    expect(_3crv_balance_after.sub(_3crv_balance_before)).to.be.eq(WAD.mul(2))
    expect(frax3crv_balance_before.sub(frax3crv_balance_after)).to.be.lt(WAD.mul(7).div(4))
    ...
    await frax3crv["remove_liquidity_one_coin(uint256,int128,uint256)"](WAD, 0, WAD.mul(18).div(10))

    frax_balance_after = await frax.balanceOf(deployer.address)
    frax3crv_balance_after = await frax3crv.balanceOf(deployer.address)

    expect(frax_balance_after.sub(frax_balance_before)).to.be.gt(WAD.mul(18).div(10))
  ...
  it("Curve stETH mock test", async function () {
    ...
    await steCRV.add_liquidity([WAD.mul(100), WAD.mul(100)], WAD.mul(199).div(2), { value: WAD.mul(100) })

    expect(await steCRV_token.balanceOf(deployer.address)).to.be.gt(WAD.mul(199).div(2))
    ...
    expect(steth_balance_after.sub(steth_balance_before)).to.be.gt(WAD.mul(9).div(10))
    ...
    expect(steth_balance_after.sub(steth_balance_before)).to.be.gt(WAD.div(110))
    expect(steCRV_balance_before.sub(steCRV_balance_after)).to.be.eq(WAD.div(100))
    ...
    await steCRV.remove_liquidity_imbalance([WAD.div(20), WAD.div(10)], WAD.mul(8).div(100))
    ...
    expect(steCRV_balance_before.sub(steCRV_balance_after)).to.be.lt(WAD.mul(8).div(100))
  ...
  it("Curve SUSD mock test", async function () {
    ...
    console.log("Adding liquidity")
    await sCRV.add_liquidity([WAD.mul(100), USDC_UNIT.mul(100), USDC_UNIT.mul(100), WAD.mul(100)], 0)
    ...
    await sCRV.remove_liquidity(WAD.div(100), [WAD.div(220), USDC_UNIT.div(220), USDC_UNIT.div(220), WAD.div(220)])
    ...
    expect(usdc_balance_after.sub(usdc_balance_before)).to.be.gt(USDC_UNIT.div(220))
    expect(usdt_balance_after.sub(usdt_balance_before)).to.be.gt(USDC_UNIT.div(220))
    ...
    await sCRV.remove_liquidity_imbalance([WAD, USDC_UNIT, USDC_UNIT.mul(2), WAD.mul(2)], WAD.mul(7))
    ...
    await sCRV_deposit["remove_liquidity_one_coin(uint256,int128,uint256)"](WAD.div(100), 0, WAD.div(110))
