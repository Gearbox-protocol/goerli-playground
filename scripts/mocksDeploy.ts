...
type SupportedEntity = SupportedToken | SupportedContract | "CURVE_STECRV_POOL" | "KOVAN_CONVEX_MANAGER";

type AddressList = Partial<Record<SupportedEntity, string>>;

const convexExtraRewardTokens: Record<ConvexPoolContract, Array<SupportedToken>> = {
    CONVEX_3CRV_POOL: [],
    CONVEX_GUSD_POOL: [],
...
async function deployCurve(
    ...
    const mainnetSteCRV_address = (contractParams.CURVE_STETH_GATEWAY as CurveSteCRVPoolParams).pool.Mainnet;
    ...
    log.info(`SUSD pool synced with params - virtualPrice: ${virtualPrice}, A: ${a}`)
    ...
    log.info(`GUSD pool synced with params - virtualPrice: ${virtualPrice}, A: ${a}`);
    ...
    log.info(`FRAX3CRV pool synced with params - virtualPrice: ${virtualPrice}, A: ${a}`);
    ...
    log.info(`LUSD3CRV pool synced with params - virtualPrice: ${virtualPrice}, A: ${a}`);
}
...
async function deployConvex(
    ...
    for (let poolToken of tokenList) {
      ...
      waitForTransaction(convexManager.syncPool(
          poolAddress,
      ...
      for (let extraRewardToken of convexExtraRewardTokens[convexData.pool]) {
          ...
          waitForTransaction(convexManager.addExtraPool(rewardTokenAddr, basePool.address))
          ...
          verifier.addContract({
            ...
            constructorArguments: [basePool.address, rewardTokenAddr, boosterAddr, convexManager.address]
          })

          const mainnetExtraPoolAddr = await mainnetPool.extraRewards(numRewards.sub(1))
          ...
          const mainnetExtraRewardStored = await mainnetExtraPool.rewardPerTokenStored();
          ...
          log.info(`Synced Convex extra pool for ${extraRewardToken} - rewardPerTokenStored: ${mainnetExtraRewardStored}`)
      }
    }
...
async function deployYearn(
    ...
    for (let yearnToken of yearnTokenList) {
        ...
        const underlyingAddress = contractAddresses[underlying] ? contractAddresses[underlying] : tokenDataByNetwork.Kovan[underlying];
        ...
        log.info(`Yearn vault for ${underlying} synced - pricePerShare: ${mainnetPPS}`);
    }
}
...
async function deployMocks() {
  ...
  const mainnetProvider = new providers.JsonRpcProvider(mainnetRpc) as SignerOrProvider;
  ...
  await deployCurve(deployer, log, verifier, contractAddresses, mainnetProvider);

  await seedCurve(deployer, log, contractAddresses);

  await deployConvex(deployer, log, verifier, contractAddresses, mainnetProvider);

  await deployYearn(deployer, log, verifier, contractAddresses, mainnetProvider);
