...
async function seedCurveTokens() {
  ...
  log.info(`ETH balance: ${await steCRV.balances(0)}, stETH balance: ${await steCRV.balances(1)}`)
  ...
  const sCRV = CurveSUSDMock__factory.connect(
      sCRV_address,
      deployer
  );
  ...
  const gusd_pool = CurveGUSDMock__factory.connect(
      gusd_address,
      deployer
  );
